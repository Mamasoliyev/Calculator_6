...
class _CalculatorState extends State<Calculator> {
  ...
  void buttonPressed(String value) {
    setState(() {
      if (value == 'C') {
        ...
        // Agar oxirgi belgi operator bo'lmasa, `EE` qo'shiladi
        if (input.isNotEmpty && !_endsWithOperator(input)) {
          input += 'EE';
        }
      } else if (value == '0') {
        // Ketma-ket 0 ni cheklash
        if (input.isEmpty || input.endsWith(RegExp(r'[+\-×÷(]'))) {
          input += '0';
  ...
  bool _endsWithOperator(String input) {
    return input.endsWith(RegExp(r'[+\-×÷(]'));
  }

  double evaluateExpression(String input) {
    try {
      // `EE` ni `*10^` formatiga almashtirish
      String formattedInput = input
          .replaceAll('×', '*')
          .replaceAll('÷', '/')
          .replaceAllMapped(RegExp(r'(\d+)EE(\d+)'), (match) {
        // Masalan: 2EE3 -> 2*10^3
